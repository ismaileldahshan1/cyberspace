...
def report_error():
    ...
    try:
        subprocess.run(["python3", "APM.py", robot, str(phase), instruction_number, error_file_path], check=True)
...
def simulate_outputs(expected_outputs):
    """Generate simulated output values based on expected output types."""
    ...
    for key, meta in expected_outputs.items():
        ...
        if var_type == "float":
            ...
            simulated[key] = "completed"
    return simulated


def run_all_phases(robot):
    """Auto-complete all phases for a robot with simulated outputs. Returns execution log."""
    log = []
    phases = plans[robot]["phases"]
    for phase_obj in sorted(phases, key=lambda p: p["phase_number"]):
        phase_num = phase_obj["phase_number"]
        expected = phase_obj.get("outputs", {})
        simulated = simulate_outputs(expected)

        # Store outputs and mark phase complete
        progress[robot]["completed_phases"].add(phase_num)
        progress[robot]["outputs"][phase_num] = simulated

        # Fill variables in next phase if needed
        next_phase = next((p for p in phases if p["phase_number"] == phase_num + 1), None)
        if next_phase:
            input_variables = next_phase.get("inputs", [])
            if input_variables:
                updated_plan = fill_in_variables(next_phase["low_level_plan"], robot)
        ...
        log.append({
            "phase": phase_num,
            "low_level_plan": phase_obj["low_level_plan"],
...
def run_mission():
    ...
    try:
        ...
        # Auto-simulate all phases for all robots
        execution_log = {}
        for robot in cfg["robots_in_curr_mission"]:
            execution_log[robot] = run_all_phases(robot)
        ...
        logging.error(f"run_mission failed: {e}")
        return jsonify({"error": str(e)}), 500
