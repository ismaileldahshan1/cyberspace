...
def analyze_error(robot, phase, instruction, error_description):
    ...
    robot_cfg = config["robots_config"][robot.upper()]
    spec_file = robot_cfg["spec_file"]
    mission_text_file = config["mission_text_file"]
    low_level_plan_file = robot_cfg["final_low"]
    ...
    **Error Description:**
    ...
    response = llm.invoke([SystemMessage(content="You are an expert in robotic planning."), HumanMessage(content=prompt)])

    return response.content.strip()

def fix_low_level_plan(robot, phase, fix_suggestion):
    """Modify the low-level plan based on the suggested fix."""
    robot_cfg = config["robots_config"][robot.upper()]
    low_level_plan_file = robot_cfg["final_low"]
    low_level_plan = json.loads(load_file(low_level_plan_file))
    failed_phase = next((p for p in low_level_plan["phases"] if p["phase_number"] == phase), None)
    ...
    **Fix Suggestion:**
    ...
    response = llm.invoke([SystemMessage(content="You are an AI that edits robotic mission plans."), HumanMessage(content=prompt)])
...
if __name__ == "__main__":
    if len(sys.argv) != 5:
        print("Usage: python APM.py <robot_name> <failed_phase_number> <failed_instruction_number> <error_file>")
        sys.exit(1)

    robot_name = sys.argv[1]
    failed_phase_number = int(sys.argv[2])
    failed_instruction_number = sys.argv[3]
